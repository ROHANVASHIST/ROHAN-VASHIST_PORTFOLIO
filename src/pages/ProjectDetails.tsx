...
export default function ProjectDetails() {
  ...
  return (
    <main className="max-w-4xl mx-auto py-20 px-6">
      <Link to="/projects" className="inline-flex items-center text-cyan-600 dark:text-cyan-400 hover:text-cyan-700 dark:hover:text-cyan-300 mb-8 font-medium transition-colors">
        <ChevronLeft className="w-4 h-4 mr-1" />
        Back to projects
      </Link>

      <h1 className="text-4xl font-bold mb-8 text-gray-900 dark:text-white">{project.title}</h1>

      {/* Image Gallery */}
      <div className="relative group mb-12">
        <div className="aspect-video w-full overflow-hidden rounded-2xl shadow-xl bg-gray-100 dark:bg-gray-800 relative">
                ...
                {images.map((_: any, idx: number) => (
                  <button
                    ...
                    className={`w-2 h-2 rounded-full transition-all ${
                      idx === currentImageIndex ? 'bg-cyan-600 w-4' : 'bg-white/60 dark:bg-white/30'
                    }`}
            ...
            {images.map((img: string, idx: number) => (
              <button
                ...
                className={`flex-shrink-0 w-24 aspect-video rounded-lg overflow-hidden border-2 transition-all ${
                  idx === currentImageIndex ? 'border-cyan-600 ring-2 ring-cyan-100 dark:ring-cyan-900/20' : 'border-transparent opacity-60 hover:opacity-100'
                }`}
              >
      ...
      <div className="grid md:grid-cols-3 gap-12">
        <div className="md:col-span-2 space-y-12">
          <section>
            <h2 className="text-2xl font-bold mb-4 text-gray-900 dark:text-white border-b dark:border-gray-800 pb-2">Overview</h2>
            <p className="text-gray-700 dark:text-gray-300 leading-relaxed text-lg">{project.description}</p>
          </section>

          <div className="grid sm:grid-cols-2 gap-8">
            <section>
              <h2 className="text-xl font-bold mb-4 text-gray-900 dark:text-white">The Problem</h2>
              <p className="text-gray-600 dark:text-gray-400 leading-relaxed">{project.problem}</p>
            </section>
            <section>
              <h2 className="text-xl font-bold mb-4 text-gray-900 dark:text-white">The Solution</h2>
              <p className="text-gray-600 dark:text-gray-400 leading-relaxed">{project.solution}</p>
            </section>
          </div>

          <section>
            <h2 className="text-2xl font-bold mb-4 text-gray-900 dark:text-white border-b dark:border-gray-800 pb-2">Key Features</h2>
          ...
          <section>
            <h2 className="text-2xl font-bold mb-4 text-gray-900 dark:text-white border-b dark:border-gray-800 pb-2">Learnings & Growth</h2>
        ...
        <div className="space-y-8">
          ...
          <section className="bg-gray-50 dark:bg-gray-900 p-6 rounded-2xl border border-gray-100 dark:border-gray-800">
            <h2 className="text-lg font-bold mb-4 text-gray-900 dark:text-white">Technologies</h2>
            <div className="flex flex-wrap gap-2">
              {project.technologies.map((tech: string) => (
                <span key={tech} className="bg-white dark:bg-white/5 text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-white/10 px-3 py-1.5 rounded-lg text-sm font-medium shadow-sm">
                  {tech}
                </span>
              ))}
            </div>
          ...
          <section className="bg-gray-50 dark:bg-gray-900 p-6 rounded-2xl border border-gray-100 dark:border-gray-800">
            <h2 className="text-lg font-bold mb-2 text-gray-900 dark:text-white">Project Status</h2>
            <div className="flex items-center">
              <span className={`w-2 h-2 rounded-full mr-2 ${project.status === 'Completed' ? 'bg-green-500' : 'bg-amber-500 animate-pulse'}`} />
              <p className="text-gray-700 dark:text-gray-300 font-medium">{project.status}</p>
            </div>
          </section>
        </div>
      </div>
